...
Mesh::Ptr Slicer::operator()( const Vec3f& p, const Vec3f& vec) const
{
    ...
    for ( int fid : fids)
    {
        const FacetCut fp( mesh, fid, p, n);
        const int nihs = fp.half();
        ...
        if ( nihs == 1)
        {
            copier.add(fid);    // All facet vertices in right half so copy
            continue;
        }   // end if
        ...
        // This will always be the case since FacetCut always calculates the intersections to be points
        // along the edges FROM vertex a.
        // If the intersecting points meet vertex a, then depending on what side of the plane the other
        // vertices are in, we either add, or reject the triangle. We can deduce this because FacetCut 
        ...
        {
            // They can't both be on the boundary since this is dealt with above
            ...
            else
            {
                ...
                // What's shortest diagonal to split the two new triangles? (yb - fp.vc()) or (yc - fp.vb())?
